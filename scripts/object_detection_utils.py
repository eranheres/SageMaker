...
def augmented_fmt_to_json(augmented, attribute_name):
    out = {}
    ...
    if augmented[attribute_name]['annotations']:
        ...
        out['annotations'] = []
    if augmented[attribute_name+'-metadata']['class-map'] and \
            augmented[attribute_name+'-metadata']['class-map']['0']:
        out['categories'] = [
                {'class_id': 0, 'name':
                    augmented[attribute_name+'-metadata']['class-map']['0']}]
    else:
...
def convert_augmented_samples_to_files_samples(
        attribute_name, bucket, augmented_manifest, output_prefix):
    images_folder = output_prefix + '/images'
    annotations_folder = output_prefix + '/annotations'
    augmented = \
        [json.loads(x)
         for x in read_s3_file(bucket, augmented_manifest).
         split('\n') if len(x) != 0]
    for aug in augmented:
        image_link = urlparse(aug['source-ref'])
        copy_s3_file(image_link.netloc, image_link.path, images_folder)
        json_annotation_file = augmented_fmt_to_json(aug, attribute_name)
        annotation_key = \
            annotations_folder[1:] + \
            '/' + \
            '.'.join(image_link.path.split('/')[-1].split('.')[:-1]) + '.json'
